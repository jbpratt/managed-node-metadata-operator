	...
	ginkgo.BeforeAll(func(ctx context.Context) {
		...
		ginkgo.DeferCleanup(func(ctx context.Context) {
			if ginkgo.CurrentSpecReport().Failed() {
				...
				err := k8s.Get(ctx, machineSetName, "openshift-machine-api", &ms)
				Expect(err).Should(BeNil(), "unable to find machineSet %s", machineSetName)

				ginkgo.GinkgoLogr.Info(fmt.Sprintf("machineset ready repliacs: %d/%d\n", ms.Status.ReadyReplicas, machinepoolReplicaCount))

				// TODO: get each machine status
				// TODO: get the operator logs
			}
		})
